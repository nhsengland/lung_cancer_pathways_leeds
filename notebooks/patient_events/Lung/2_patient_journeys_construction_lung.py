...
# MAGIC %md
# MAGIC # 2. Lung Cancer Patient Flag Creation
# MAGIC This notebook contains tables compiling pontential lung cancer flags which appeared in patients' records prior to there diagnosis with lung cancer. For example, if a patient reported shortness of breath to their GP 5 months before diagnosis then this will be recorded here. 
# MAGIC Relevant events such as chest x-rays or GP referrals for suspected cancer are also recorded.
...
df_cancer_site = read_parquet_file(containerName =config.containerName_platinum, 
                                   lakeName=config.lakeName,
                                   filePath= f""
...
df_copd_summary = patient_pathways_utils.count_and_first_date_of_event(df_event_table=df_copd_combined,
                                                                       history_days = history_days*copd_years_history, # COPD diagnosis in last years_history years
...
df_patient_flags = df_cancer_site.join(df_latest_cohort.drop("diagnosis_date_earliest"),
                                       on = "Patient_ID",
                                       how="inner")
...
df_patient_flags = df_patient_flags.withColumn(
    "tumour_stage_group",
    F.when(F.col("tumour_stage_earliest_first_char").isin("1", "2"), "early")
     .when(F.col("tumour_stage_earliest_first_char").isin("3", "4"), "late")
     .otherwise("unknown")
)
...
for milestone_date in milestones:
    ...
    # identify xrays occurring upto 10 days before the milestone date, and thereafter
